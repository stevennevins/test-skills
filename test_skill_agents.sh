...
# Test skill frontmatter behavior: context (fork/omitted) and agent (general-purpose/omitted)
# Compares local skills vs plugin-installed skills
# Requires: claude CLI, simplify-plugin installed

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$SCRIPT_DIR"

# Setup: create test file and git repo if needed
setup() {
...
TSEOF
  ...
  git commit -m "Add user-service with intentional quality issues for skill testing"
...
# Launch claude with the full test prompt
exec claude --permission-mode default "$(cat <<'PROMPT'
We are testing how skill frontmatter settings affect behavior. The secret keyword is "banana".

There are 8 simplify skills to test (4 local, 4 plugin) plus a context-probe skill.
Each skill reviews src/user-service.ts for code quality issues.

Run each skill below ONE AT A TIME. After each skill completes:
1. Note the behavior: did it run inline (as instructions in this conversation) or fork (autonomous subagent)?
2. If it ran inline, do NOT actually execute the review — just note "INLINE" and move on
3. If it forked, note what the subagent returned
4. Reset the file: git checkout -- src/user-service.ts

## Test sequence

### Local skills (defined in .claude/skills/)
1. /simplify-default        — no context, no agent
2. /simplify-agent-only     — agent: general-purpose, no context
3. /simplify-fork-only      — context: fork, no agent
4. /simplify-fork-agent     — context: fork, agent: general-purpose

### Plugin skills (installed from simplify-plugin)
5. /simplify:plugin-simplify-default        — no context, no agent
6. /simplify:plugin-simplify-agent-only     — agent: general-purpose, no context
7. /simplify:plugin-simplify-fork-only      — context: fork, no agent
8. /simplify:plugin-simplify-fork-agent     — context: fork, agent: general-purpose

### Context isolation test
9. /context-probe  — context: fork, tests whether forked subagent can see "banana"
...
- context setting
- agent setting
- Actual behavior (inline/forked)
- Could see "banana"? (for context-probe only)

Then note any differences between local and plugin behavior.
PROMPT
)"
